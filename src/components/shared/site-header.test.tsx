...
describe("SiteHeader primary nav prefetch", () => {
  ...
  it("keeps default prefetch for other primary nav links", () => {
    render(<SiteHeader isAuthenticated={false} role={null} hiddenNavHrefs={[]} />);

    const requestsLink = linkPrefetchByHref.find((entry) => entry.href === "/requests");
    expect(requestsLink).toBeDefined();
    expect(requestsLink?.prefetch).toBeUndefined();
  });
});
