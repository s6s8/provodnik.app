...
describe("PublicRequestsMarketplaceScreen", () => {
  ...
  it("filters category pills by request interests", () => {
    ...
    const thematics = screen.getByText("Тематика").parentElement;
    expect(thematics).not.toBeNull();
    fireEvent.click(
      within(thematics!).getByRole("button", { name: "Гастрономия" }),
    );

    expect(screen.getByRole("link", { name: /Казань/ })).toBeInTheDocument();
    expect(screen.queryByRole("link", { name: /Тбилиси/ })).not.toBeInTheDocument();
  });
});
