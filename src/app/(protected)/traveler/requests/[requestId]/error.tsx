...
type TravelerRequestErrorProps = {
  error: Error & { digest?: string };
  reset: () => void;
};

export default function TravelerRequestError({ error, reset }: TravelerRequestErrorProps) {
  useEffect(() => {
    console.error(error);
  }, [error]);
      ...
      <div className="bg-glass backdrop-blur-[20px] border border-glass-border shadow-glass rounded-glass w-full max-w-2xl space-y-6 px-6 py-8 md:px-10 md:py-10">
        <div className="space-y-3">
          ...
          <p className="max-w-xl text-sm text-ink-3 md:text-base">
            Попробуйте повторить действие или вернуться к списку запросов.
          </p>
        </div>
