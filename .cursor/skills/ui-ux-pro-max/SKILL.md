...
**Cursor install path:** Scripts live under `.cursor/skills/ui-ux-pro-max/scripts/`. Run commands from the repo root that contains that folder (`provodnik` orchestration root or `provodnik.app`). Use `python` on Windows; use `python3` on macOS/Linux if that is the available command.
