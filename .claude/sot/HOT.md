...
### AP-025 / ERR-079 — Terminal state must notify the worker
**Never** write `state='ABORTED'` / `state='ABANDONED'` (or any terminal state) and rely on cleanup logic alone — the worker watching the session topic sees a bot that just went silent and assumes it's broken. Workers do not read the audit topic; that's owner territory. Any silent terminal is a mission failure (Asymmetry #3: portfolio coherence — the worker is forced to become PM-of-the-bot). Surfaced 2026-05-13 by Alex on session K4 `ypwv` topic 370: "заморозили и даже не отрапортовали об этом".
**Always** route every terminal transition through `drivePipeline` (so the ERR-079 terminal-notify block fires in `bot/pipeline/driver.mjs`) AND capture a human-readable reason at the abort surface. The `/override <sid> abort [reason]` and `/resume <sid> abort [reason]` regexes now take optional trailing reason → `session.abortReason`. If you add a new handler that writes terminal state, it MUST call `continuePipeline(sid)` after the save so the terminal-notify block runs.
**Bonus rule:** every new field on the session object MUST also be declared in `bot/pipeline/state/session-schema.mjs` Zod schema as `.optional()` — otherwise `safeParse` silent-strips it on load (ADR-061). ERR-079 only works because `abortReason` and `terminalNotifiedAt` were added to the schema in the same commit.

---

### AP-026 / AP-027 / AP-028 — Worker UX after Alex's 2026-05-13 usability run
**Never** rely on `answerCallbackQuery` toast alone for gate feedback — workers don't see the toast on mobile, tap again, fire stale-callback 400s. **Always** edit the gate message: stamp `✅ <action> · @<user>` in italic + omit `reply_markup` so the keyboard disappears (Telegram removes it on edit when the field is absent). Best-effort try/catch; ignore "message is not modified".
**Never** wrap short paste-and-run commands in ```triple-backtick``` blocks — some Android Telegram builds copy that as an image. **Always** use `` `inline code` `` (single backtick) for one-line commands. Reserve triple-backtick for genuinely multi-line code/config. Codified in `bot/lib/{epic,think}-runner.mjs` system prompt Telegram-formatting section.
**Never** lead worker-facing messages with full sessionIds like `20260513-160-px-96-px-g00m` — 30+ chars, impossible to keep in working memory across tickets. **Always** allocate a short ID at intake (T-NN sessions, E-NN epics) from the per-app monotonic counter at `<app>/.short-id-counter` (file-mutex guarded, see `bot/lib/short-id.mjs`). Topic names lead with the short ID. Full sessionId stays in JSON state for owner debug + git correlation but never leads in copy.
...
### AP-031 / ERR-082 — audit findings must reach downstream nodes' KNOWLEDGE
**Never** decompose with numeric counts or literal identifier lists baked into ticket titles when those numbers/lists are supposed to come from a dependent audit/research/inventory/survey-typed node. The decomposition validator now hard-rejects "Add 8 slugs"/"Create 5 chips"/"Unify chips:[a,b,c]" patterns when the deps include an audit-typed node (3-retry → escalate). And: every ticket whose deps include a node that shipped a markdown artifact gets that artifact auto-inlined under "Dep-shipped artifacts" in cursor-agent's KNOWLEDGE (6K char cap). Plus plan-critique runs audit-coverage-check on top of model PASS — extracts file mentions from audit-style sections ("Cross-Surface Contradictions" / "Findings" / "Affected Files"), escalates if plan.fileScope ignores them without rationale acknowledgment. **E-4 #2 (canon-without-audit) is the canonical example of this whole pipeline failing once** — shipped a renamed INTEREST_CHIPS as "canon" while the audit found 7 listing labels + religion drift + dual-column trigger. Don't repeat.

### AP-032 / ERR-082 — confidence=1 / concerns=[] on a high-stakes path = manual gate required
**Never** trust the deterministic "all checks green" PASS from 15-verify-decide.mjs when plan.fileScope touches schema (`supabase/migrations/`), data layer (`src/data/`), config (`*.config.*`), or orchestrator state/store modules (`bot/pipeline/state/`, `bot/lib/*-store.mjs`). Verify steps test code-as-written, not code-as-needed. bot/lib/high-stakes-paths.mjs::isHighStakesPath() classifies; 15-verify-decide.mjs forces confidence down to 0.7 with appended concern when the deterministic PASS would otherwise fire. Owner sees the SHIP_GATE prompt and decides. Don't expand the high-stakes list without justification — adding a pattern makes shipping slower for that surface.
