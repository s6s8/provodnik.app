...
### ERR-059: tripster-v1 e2e specs reference seed users + selectors that never matched reality — RESOLVED 2026-06-12 (suite green vs dev)
- **Resolution (2026-06-12):** (1) Fixture identities centralized + env-driven: `tests/e2e/fixtures.ts` builds SEED_USERS from `QA_SEED_PASSWORD` env with `qa-{admin,guide,traveler}@example.com` (matching `scripts/seed-test-users.mjs`, which now supports a fixed password); `E2E_READY` exported. (2) All 6 specs un-gated: unconditional `test.skip` replaced with conditional `test.skip(!E2E_READY, ...)` runtime guards. (3) Login helper hydration-safe (refill until submit button enables — values typed pre-hydration get wiped; `networkidle` useless under dev HMR). (4) `playwright.config.ts`: `E2E_BASE_URL` env for remote targets, 90s budget + 1 retry there (dev compile latency). **Green run vs dev.provodnik.app: exit 0 — booking-confirm + dispute flows executed end-to-end; remaining skips are legitimate state guards.** Internal-form selector rot (layer 2/3) still applies to specs 01/03 deeper flows — they self-skip; modernizing them to current product flows is follow-up, not rot.
- **Symptom (historical):** All 6 specs in `tests/e2e/tripster-v1/` (01-create-listing through 06-dispute-flow) fail at the auth step with "Неверный email или пароль." (invalid credentials), even with the auth selector fix from commit `f5dc1ca`. Surfaced during Phase 4 of the macmini migration when `bun run playwright` was wired up via webServer auto-boot.
- **Root Cause (3 layers):**
...
### ERR-091: the canonical theme list was built from the wrong vocabulary (photo vs religion)
- **Symptom:** `src/data/themes.ts` — the single canonical 8-theme list every product surface derives from — had slot 8 as `photo`/«Фотопрогулки». It should have been `religion`/«Религия». Caught by the worker (Alex) after the whole E-10 epic shipped: "Восьмой пункт — Фотопрогулки, а не Религия... В форме лежит religion. В каноне — photo. Канон взял список с Гидов, а не с формы."
- **Root Cause:** The agreed source of truth for the 8 themes was the **traveler request form** (`homepage-request-form.tsx`, `INTEREST_OPTIONS`), which carries `religion`. The canon module (created by old E-4 #2) instead copied the **Guides-page** vocabulary (`INTEREST_CHIPS`), which carries `photo`. Seven slugs matched; the 8th came from the wrong surface. The original E-4 #1 audit compounded it by labelling `religion` as "drift" (present in the form, absent from the Guides list + the DB CHECK) — so every subsequent stage, and the operator, trusted that and the entire E-10 epic propagated `photo` and actively *eliminated* `religion` (E-10 T2 even made the request Zod schema reject `religion` at parse time). Nobody examined whether the canon's 8 slugs were the right 8 — only how to consolidate onto them.
- **Fix:** quantumbek-side N/A (this is provodnik product code). provodnik `41cbeb7` + migration `20260516000001`: (1) `themes.ts` slot 8 `photo`/Camera → `religion`/Church + the `ThemeSlug` type; propagates automatically to the request form, request schema, guide profiles, `/guides` filter+search and listings because E-10 had already routed every surface through `THEMES`. (2) `public-listings/mapper.ts` — `THEME_SLUGS` now derives from `THEMES` (it had been a hand-copied list — the same drift class); `Фотография`/`Фотопрогулки`→`photo` legacy entries dropped, `Религия`→`religion` added. (3) Migration: `guide_specializations_valid` CHECK swapped `photo`→`religion`, after stripping stale `photo` values from `guide_profiles.specializations` + `traveler_requests.interests`. Applied to prod via the Supabase Management API; constraint verified.
- **Files Affected:** `provodnik.app/src/data/themes.ts`, `src/data/public-listings/mapper.ts`, `supabase/migrations/20260516000001_themes_canon_religion.sql`.
- **Date:** 2026-05-16
- **Prevention:** AP-035. A "canon" / single-source-of-truth constant is only as correct as the surface it was copied from — when establishing one, the FIRST question is *which surface is authoritative*, decided explicitly, not inferred from whichever list happened to be audited first. An audit that flags a value as "drift" must answer *which side is the drift* — the value or the canon — not assume the canon. Derive secondary lists from the canon (never hand-copy) so they cannot diverge.


### ERR-092: migration files in the repo were never applied to the live DB
- **Symptom:** Two production flows were silently broken. (1) Creating a traveler request on `/traveler/requests/new` failed server-side with `column traveler_requests.date_flexibility does not exist` (Postgres 42703) — every submit, every mode. (2) Guide search on `/guides` had no `search_guides` RPC to call. Both surfaced 2026-05-19 during a live browser walk of the command-center epic — not by any test (200 unit/integration tests were green throughout).
- **Root Cause:** Migration *files* existed in `supabase/migrations/` but were never applied to the live DB (`yjzpshutgmhxizosbeef`): `20260502000002_add_date_flexibility.sql` (adds the `date_flexibility` column) and `20260516000001_search_guides_rpc.sql` (the `search_guides` RPC + `guide_search_result_row` view + `guide_profiles_languages_gin` index). The `supabase_migrations` tracking table is unreliable — migrations were applied ad-hoc (SQL editor / dashboard) outside `supabase db push`, so the table both under-reports applied migrations and its version numbers do not match the repo filename prefixes. A file existing in `supabase/migrations/` is NOT evidence it ran.
- **Fix:** Both migrations applied 2026-05-19 via the Supabase MCP `apply_migration`. `date_flexibility` — additive `ADD COLUMN IF NOT EXISTS … DEFAULT 'exact'`; verified by an end-to-end browser submit (assembly mode, empty groupMax → request saved + read back). `search_guides_rpc` — function + view + GIN index; verified `search_guides('')` and `search_guides('москва, история')` execute cleanly and `/guides?q=` returns 200. A full audit of all 40 migration files vs the live schema confirmed these were the only two genuine schema gaps; data migrations (`reseed_listings`, `fix_listing_images`, `specializations_backfill`, `themes_canon_religion`) were deliberately NOT re-run.
- **Files Affected:** `supabase/migrations/20260502000002_add_date_flexibility.sql`, `supabase/migrations/20260516000001_search_guides_rpc.sql` (both repo files; the gap was applying them to the DB, not the files themselves).
- **Date:** 2026-05-19
...
### ERR-095 → RESOLVED 2026-05-22
- **Symptom:** During E-32 T1 cxsc dispatch, CONSISTENCY stage escalated (safety, conf 0) on a clean additive diff — flagged AP-012 «inline currency math» on `src/lib/supabase/supabase-client.ts`. The diff was strictly additive: 5 new field maps, 5 new SELECT columns including `budget_minor`, 5 payload defaults; no `* 100`, no `/ 100`, no inline arithmetic anywhere. Safety escalations cannot be `/override`-proceeded → genuine blocker → forced an orchestrator hotfix mid-epic.
- **Root Cause:** The AP-012 regex in `apps/provodnik/config.json` was `\*\s*100\b|\/\s*100\b|\b\w*_minor\b`. The bare `\b\w*_minor\b` alternative matches any occurrence of a `_minor`-suffixed token regardless of arithmetic context — so a `select('id, budget_minor, …')` string in supabase-client.ts triggered the rule with zero arithmetic actually present. The `excludeFilePattern` covered money.ts/types.ts/tests but NOT data-layer files like supabase-client.ts which legitimately reference `_minor` columns by name in queries. Every additive diff touching a SELECT with a `_minor` column would have hit this — T2-T6 would have hit it too.
- **Fix:** Narrowed the regex to require an arithmetic operator adjacent to the `_minor` token: `\*\s*100\b|\/\s*100\b|\b\w*_minor\b\s*[*/]|[*/]\s*\b\w*_minor\b`. RED→GREEN verified: the old pattern matched the SELECT line (bug); the new pattern does NOT match SELECT lines, payload field-maps, or column-name references, but STILL matches `_minor * rate`, `record._minor / 100`, `value*amount_minor`. Hotfix committed live to mini `apps/provodnik/config.json` only (gitignored per-machine config per orchestrator design), restart 73 clean.
- **Files Affected:** `apps/provodnik/config.json` (mini-only, not in canonical repo by design — orchestrator/.gitignore excludes `orchestrator/apps/*/config.json` as per-machine state)
- **Date:** 2026-05-22
- **Prevention:** A consistency rule whose pattern matches outside the rule's intended context is worse than no rule — it forces orchestrator-direct hotfixes mid-epic for clean diffs. See AP-037. When authoring a new regex rule, mentally walk it against: legitimate column references in SELECT strings, type declarations (`column: number`), comments, JSDoc — not just arithmetic positions. The minimum precision bar for a `_minor`-class column rule is **adjacency to an arithmetic operator**.


### ERR-096 → RESOLVED 2026-05-23
- **Symptom:** Live login on production looked like "cookies don't persist": after typing valid creds at /auth, the page briefly showed a loading state, then bounced back to /auth (or to /auth?error=missing-role on a second attempt). Navigating to /traveler/requests directly after a successful sign-in returned ERR_TOO_MANY_REDIRECTS in the browser. Affected every user account created outside the public `signUpAction` flow — seed scripts, admin-tooling-created users, direct DB inserts. Users created via the public sign-up form were unaffected.
- **Root Cause:** Three places in the codebase read the user's role from two disagreeing sources. The client-side login flow (`auth-entry-screen.tsx`) gated the post-login redirect on `data.user.app_metadata.role`. The Next.js proxy (`src/proxy.ts`) gated every protected-route access on `session.user.app_metadata.role`. The server-side auth context (`src/lib/auth/server-auth.ts`'s `readAuthContextFromServer`) read role from the `profiles.role` column. `signUpAction` stamps `app_metadata.role` as part of the signup transaction, so all three agreed for canonical users. Users created any other way only had `profiles.role` populated. The two halves disagreed: proxy bounced the user to `/auth?error=missing-role` (no JWT claim), `/auth` SSR read profile.role and set `canonicalRedirectTo: /traveler/requests`, bounced them back → infinite loop. Earlier diagnosis of "login drops cookies" was misreading the symptom — the client-side fallback at `auth-entry-screen.tsx:130` was firing `window.location.href = "/api/auth/signout"` for the no-role case, which DID clear cookies and made the bug look like a cookie persistence issue.
- **Fix:** Aligned client and proxy to the same source-of-truth pattern as `readAuthContextFromServer`. Both now try JWT `app_metadata.role` first (the synchronous fast path stamped by `signUpAction`); if absent or invalid, fall back to a one-shot `profiles.role` lookup. Cost: one DB query per protected route for users without the JWT claim. Canonical signup users still hit the fast path. Two-commit fix: `5fcbfbe` patches `src/features/auth/components/auth-entry-screen.tsx`; `5eef6f3` patches `src/proxy.ts`.
- **Files Affected:** `src/features/auth/components/auth-entry-screen.tsx`, `src/proxy.ts`
- **Date:** 2026-05-23
