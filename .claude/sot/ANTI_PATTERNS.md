...
### AP-036: react-hook-form `valueAsNumber: true` on an OPTIONAL numeric field
- **What was tried:** `register("groupMax", { valueAsNumber: true })` on the optional «Максимум в группе» input in both `traveler-request-create-form.tsx` and `homepage-request-form.tsx`. The schema declares `groupMax: z.number().int().min(1).max(50).optional()`.
- **Why it failed:** With `valueAsNumber: true`, an empty input coerces to `NaN`, not `undefined`. `z.number()` rejects `NaN`, and `.optional()` only permits `undefined` — so a left-blank optional field fails validation. The whole form submit is then silently blocked: no server call, no error toast, nothing. The bug stayed latent while the form defaulted to `private` mode (where `groupMax` is never rendered, so the field kept its `undefined` default); flipping the default to `assembly` mode rendered the empty field and exposed it. Caught by `homepage-request-form.test.tsx` whose submit-path tests began failing — the traveler form had no equivalent test and would have shipped broken.
- **Correct approach:** For an optional numeric input, map empty → `undefined` with `setValueAs`: `register("groupMax", { setValueAs: (v) => (v === "" || v == null ? undefined : Number(v)) })`. `valueAsNumber: true` is only safe on a *required* numeric field, where `NaN` correctly fails as "required". Note `Number("")` is `0`, so the empty-string check must precede the `Number()` call.
- **Sibling APIs to audit:** every `register(…, { valueAsNumber: true })` whose field is `.optional()` in its zod schema — `grep -rn "valueAsNumber" src/` and cross-check each registration against its schema.
- **Cycle:** 2026-05-19 (command-center epic — request-form default flipped to «Сборная группа», exposing the latent NaN).


### AP-037: Consistency-rule regex matches the symbol outside its arithmetic context
- **What was tried:** A regex consistency rule that flags currency arithmetic outside the centralised money helpers (AP-012) was written as `\*\s*100\b|\/\s*100\b|\b\w*_minor\b`. The third alternative `\b\w*_minor\b` was intended as a catch-all for any direct `_minor` token usage outside `money.ts`.
- **Why it failed:** The pattern matches any `_minor`-suffixed identifier regardless of arithmetic context: SELECT column strings (`select('id, budget_minor, …')`), type declarations (`budget_minor: number`), JSDoc references, comment mentions — none of which are AP-012 violations. The `excludeFilePattern` was sized for known helper paths but every data-layer file (e.g. `supabase-client.ts`, `requests.ts`) legitimately references `_minor` columns by name and would trigger the rule on any additive diff. Result: clean diffs blocked at CONSISTENCY stage with no `/override` path (safety escalations gate-out). See ERR-095 for the live incident.
- **Correct approach:** A regex rule must encode the FORBIDDEN OPERATION, not just the forbidden SYMBOL. For `_minor` arithmetic specifically: require an arithmetic operator (`*`, `/`) adjacent to the token: `\b\w*_minor\b\s*[*/]|[*/]\s*\b\w*_minor\b`. For other rules: anchor on a syntactic feature that proves the operation is present (function call, operator, assignment), not just the noun. Before adding any rule whose pattern matches a domain noun, mentally walk it against: legitimate column references, type declarations, comments, JSDoc, fixture data. If those produce false positives, the rule is too loose.
- **Sibling APIs to audit:** every `consistencyRules[].pattern` in `apps/*/config.json` whose pattern is symbol-anchored rather than operation-anchored — re-evaluate against legitimate non-arithmetic surfaces.
- **Cycle:** 2026-05-22 (E-32 T1 cxsc — clean additive diff blocked mid-epic; orchestrator hotfix forced).


### AP-038: Reading the user's role from JWT `app_metadata.role` alone (without a `profiles.role` fallback) for an auth decision
- **What was tried:** Client-side login flow in `auth-entry-screen.tsx` and the Next.js proxy in `src/proxy.ts` both gated on `session.user.app_metadata.role`. The signup path (`signUpAction`) stamps this field as part of the user-create transaction, so the JWT claim is the natural fast path. The proxy used it as the sole source of truth for protected-route access; the client login used it to decide which dashboard to redirect to.
- **Why it failed:** Not every user-creation path stamps `app_metadata.role`. Seed scripts, admin-tooling user creation, direct DB inserts — all of these populate `profiles.role` (which is the canonical source read by the server-side `readAuthContextFromServer`) but skip the JWT claim. For those users, the two halves of the auth system disagreed: proxy/client said "no role, bounce them out", server-auth said "role is `traveler`/`guide`/`admin`, send to dashboard". The result was an infinite redirect loop between `/auth?error=missing-role` and the dashboard route — eventually surfaced to the user as ERR_TOO_MANY_REDIRECTS. See ERR-096 for the live incident.
- **Correct approach:** Treat `profiles.role` as the canonical source. JWT `app_metadata.role` is a fast-path cache stamped by canonical signup. Any auth checker that reads role for a decision must follow this pattern: try the JWT claim first (synchronous, free), fall back to a one-shot `profiles.role` lookup when the claim is absent or invalid. Pattern lives in `src/lib/auth/server-auth.ts` (`readAuthContextFromServer`) — replicate it in any new auth touchpoint, including server actions that gate by role.
- **Sibling APIs to audit:** every `session.user.app_metadata?.role` or `data.user?.app_metadata?.role` reference in the codebase — `grep -rn "app_metadata?.role\|app_metadata\.role" src/`. Each one is a potential repeat of this bug if it gates a decision without a fallback.
- **Cycle:** 2026-05-23 (ERR-096 — auth-flow regression surfaced during BUG-2 live verification; client + proxy fixed in `5fcbfbe` + `5eef6f3`).

### AP-039: Verification clicks on react-hook-form controlled inputs via JS `input.click()` produce false-positive results
- **What was tried:** Automated verification flow used `document.querySelector('input[type=radio][value=private]').click() + dispatchEvent(new Event('change', {bubbles:true}))` via Chrome DevTools `evaluate_script` to switch a react-hook-form-controlled radio group between two modes, then read `body.innerText` for assertions and took a screenshot.
- **Why it failed:** A native `click()` on a controlled radio input sets `input.checked = true` at the DOM level momentarily and fires a native `change` event — but the React synthetic event chain that react-hook-form's `<Controller>` / `register` machinery listens to does NOT receive an equivalent. React's controlled-input pattern re-asserts the previous value on the next render, so the radio bounces back to its bound state. `input.checked` reads `true` for one microtask (so JS-level assertions like `radio.checked === true` succeed), but React state never updates, the dependent UI never re-renders, and the screenshot captures the original mode. The diagnostic returns success while the visible reality is unchanged — false-positive verification.
- **Correct approach:** For verification clicks on react-hook-form (or any React-controlled) inputs, click the LABEL or wrapping `StaticText` element via the dedicated automation tool's UID-based `click` (Chrome DevTools `click` by `uid`, Playwright `page.getByLabel(...).click()`, etc.). The label's click goes through the full event chain including React's synthetic events, RHF receives the change, state updates, UI re-renders before the next take_screenshot. Never use `evaluate_script` direct `input.click()` for controlled inputs when the goal is a visual verification.
- **Cycle:** 2026-05-23 (sent two side-by-side screenshots claiming to show assembly vs private mode on the traveler request form — both screenshots showed assembly mode because the JS-level click never propagated to RHF. Owner flagged the duplicate visual. Re-captured by clicking the label `StaticText` via the snapshot UID — produced the correct private-mode capture).
