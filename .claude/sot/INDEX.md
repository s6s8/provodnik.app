...
- ERR-063 — orchestrator verify-decide stage ignores e2e=false in verify results; advanced to SHIP_GATE on a session where playwright reported 5 spec failures (current of 5 in scope), claiming "Typecheck and lint both passed cleanly" with confidence 0.82. Fix scope: bot/pipeline/stages/15-verify-decide.mjs must read ALL fields of verify.results (typecheck, lint, test, e2e, build) and ESCALATE if ANY is `false` (not skipped). Currently produces wrong PASS signal that masks real failures and lets broken work reach SHIP_GATE. Surfaced 2026-05-10 Phase 6.4 session 20260510-fix-err-059-tripster-v0lg. OPEN
- ERR-062 — orchestrator dispatch stage's diff capture is empty even when cursor-agent edited files. Root cause: `bot/lib/git-ops.mjs:21` runs `git diff main...HEAD` (three-dot, committed-only). Cursor-agent edits files but never commits per skeleton.md `<shell_ban>` design — orchestrator is supposed to commit AFTER. Result: diff captured before commit returns empty. Compounds ERR-061 (consistency advances on empty diff). Fix scope: 12-dispatch.mjs must (a) commit cursor-agent's worktree changes via git-ops before getWorktreeDiff, OR (b) getWorktreeDiff must use `git diff main` (working-tree vs main, includes uncommitted). Surfaced 2026-05-10 Phase 6.4 sessions 135 + 141 — work was correct in worktree, orchestrator saw 0-line diff. OPEN
- ERR-061 — orchestrator advances past consistency stage on EMPTY DIFF without escalation. Phase 6.4 first dispatch: cursor-agent reported DONE with 1-line diff (header only, zero changes); consistency stage correctly flagged "empty — no lines added or removed... orchestrator should verify whether an empty diff was intentional or indicates a pipeline error upstream"; but driver advanced to verify (which passed trivially on empty diff) and then SHIP_GATE. Owner clicked Cancel after manual inspection. Compounds ERR-049 (cursor-agent hallucinated DONE). Fix: 13-consistency.mjs should ESCALATE when diff is empty AND fileScope was non-empty, unless explicitly a no-op ticket. OPEN
- ERR-060 — orchestrator RESEARCH stage lacks codebase awareness; passes only HOT.md + CLAUDE.md + ticket text to claude, no source excerpts, no Glob/Grep/Read tool access. Symptom: any vague-but-real ticket ("test the homepage") escalates for missing source paths the stage never provides. Compounded by Gap B (interpretation ambiguity — "test" has 6+ valid meanings) + Gap C (no action-defaults SOT entry injected) + Gap D (tool-access feasibility under `claude --print + --json-schema` untested). Surfaced 2026-05-10 during Phase 6.4 first real ticket (session 20260510-do-smoke-test-files-5kvv, cancelled). Fix scope = Phase 7: REPO_MAP.md SOT entry + claude CLI tool-access spike + PATTERNS.md inclusion in RESEARCH prompt + escalation rule split. OPEN
