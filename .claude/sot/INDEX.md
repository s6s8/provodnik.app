...
- ERR-069 — orchestrator's `/think-cancel` slash command appeared to be silently ignored when sent via tg-cli on 2026-05-12 — the message landed in the topic (msg_id captured) but no bot reply, no state flip, no closeForumTopic call, no stderr trace. Re-sending plain text in the same topic correctly routed to handleThinkTurn (state intact, OPEN session matched), so onTopicMessage was firing — the `text === '/think-cancel'` predicate at bot.mjs:887 didn't match. Suspected: grammY rewrites command text mid-dispatch (possibly @-suffix appended), or text trim() is stripping a control char. UNVERIFIED — workaround used: manually flip .thinks/*.json `state` to CANCELLED via jq. Reproduce by sending `/think-cancel` from tg-cli (auth as owner) into an OPEN think-topic; expect "Brainstorm cancelled" reply + closeForumTopic, observed no reaction. OPEN — investigation needed.
