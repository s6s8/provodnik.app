...
- ADR-063 — /form is the canonical MANUAL request form (icon-toggle layout). Homepage `/` is the AI/conversational flow; `/form` is the manual fallback linked from it (homepage Обычная форма link). Canonical look: compact UserPlus icon group-mode toggle next to Сколько вас + tooltip, budget-per-person inside the group row, 3-themes + Ещё темы collapse kept. Recover from commit `fa41b912` (PR #125): `git checkout fa41b912 -- "src/app/(home)/form/page.tsx" src/features/homepage-classic/components/homepage-request-form-classic.tsx`; ensure NO /form -> / redirect in next.config.ts; homepage Обычная форма link -> /form. Reverses the /form portion of IA refactor #124 — do not re-collapse. Data path stays on @/features/requests/create-request-actions. — 2026-06-14
...
- **ERR-101** Russia mobile-carrier reachability: `provodnik.app` reachable from RU datacenter ASNs (check-host.net RU nodes 200 OK across HTTP/TCP/ping/DNS) but reportedly blocked on consumer mobile carriers (MTS/МегаФон/Tele2/Yota). Domain NOT in RKN reestr (scanned all 20 zapret-info shards + nxdomain.txt = 0 hits). Cloudflare+Vercel infra clean (no firewall, no geo-rule, 78 RU req / 0 challenges in 12h). Block is carrier-side heuristic, not RKN-official. Hygiene fixes applied (apex CNAME to Vercel dynamic, min TLS 1.2, always-HTTPS on); three diagnostic subdomains live (m./app./ru.provodnik.app) for mobile testing; alt .ru mirror identified (provodnikapp.ru / provodnik-app.ru both AVAILABLE). Full investigation: codex-ops/state/handoffs/provodnik/ru-block-investigation-2026-05-27.md (OPEN — pending mobile-phone test) -> ERRORS.md (2026-05-27)
