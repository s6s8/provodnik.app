...
| Комиссия | 22% + платное продвижение | 0% в v1 |

Готовые экскурсии у Проводника есть — но листинг это **витрина и сигнал
доверия**, не продукт бронирования. Продукт бронирования — отклик на
запрос. Ловушка: request-first — это дифференциатор; паритетные фичи
(каталог, фильтры, страницы листингов) поддерживают его, но не строятся
ВПЕРЁД петли запрос → отклик → присоединение. Принцип MVP:
«request-first, but not request-only» (`MVP.md` §5).

### MVP — модель 1 из 3

MVP Проводника (**модель 1 из 3**) — кастомизированные запросы на сборные
группы, НЕ готовые экскурсии. Точка входа — запрос путешественника на
сборную группу; готовые экскурсии — фоновый инвентарь гида, не двигатель
воронки. Экономика: сборная группа снижает цену на человека, поэтому в
неё входят те, кому одиночная экскурсия не по карману (источник: Alex,
обсуждение 2026-05-17). Модели 2 и 3 — НЕ определены; см. `NEXT_PLAN.md`.
...
- 0% комиссии — ценность платформы в чистом матчинге.
...
- 2026-05-17 [verification] — Перед нетривиальным утверждением о коде, архитектуре, состоянии ветки или поведении функции — сверься с исходником, файлами и историей коммитов; по справочным вопросам ответ по памяти допустим.
