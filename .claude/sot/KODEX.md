...
**Счётчик участников на карточке (канон 2026-06-09).** Число в поле
«кол-во участников» при создании запроса = **фактические участники**
(автор + его компания), а не желаемый минимум и не вместимость.
Символ `≥` у «Сборная группа» означает «группа открыта для
присоединения», а не «минимум N человек». Счётчик берётся из этого
поля формы. Аватары в строке 5 карточки показывают реальных
вступивших. `max_participants` — поле **гидской ставки**, не запроса.
...
- 2026-06-07 [analysis] — Full-picture-first: before answering any "what is X / what's the difference / do we need this" question, grep all callsites/usages. Answer must include: what it is → why it exists → who calls it → what breaks if removed → recommendation. One reply, no follow-up extraction required from Alex.
- 2026-06-07 [analysis] — Verify-before-propose: never say "can remove / can delete / can retire" until callsites are confirmed clear. Order is check → propose. Propose → check is wrong order and a wrong answer.
- 2026-06-07 [communication] — Don't ration context. Holding back the "why" to stay brief is worse than being complete. If the full picture fits in one reply, it goes in one reply.
