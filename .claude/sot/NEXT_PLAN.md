...
- **PENDING-1: Канонический термин «Открытая группа»** — заменяет «Сборная группа»
  везде в UI, лейблах, форме запроса, карточках. Зафиксировано в `KODEX.md`.
  Acceptance: терминология-sweep по всему интерфейсу; `mode: "assembly"` в коде
  не трогаем (техническое значение, не UI-лейбл).

- **PENDING-2: Форма запроса — половинные поля «Сколько вас» + «Открытая группа»**
  — поле groupSize (число) и чекбокс «Открытая группа» объединяются в одну строку
