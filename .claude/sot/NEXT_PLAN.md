...
## Pending decisions — ВСЕ ВЫПОЛНЕНЫ 2026-06-13 (Go дан владельцем 2026-06-12, «fix everything»)

- **PENDING-1 — ГОТОВО (2026-06-13).** Терминология «Сборная группа»/«Своя группа»: последний
  остаток «Открытой группы» убран из public-request-detail-screen (job group-term-sweep, влит в
  main). Навигация «Открытые группы» (каталог открытых запросов) сознательно НЕ тронута — другой
  термин; переименование — отдельное решение владельца. `mode: "assembly"` не тронут.

- **PENDING-2 — ГОТОВО (2026-06-13).** «Сколько вас» + «Сборная группа» в одной строке half-width
  на живой форме /form (job form-layout-pending234-repair, влит; operator сам отремонтировал
  заблокированную первую попытку).

- **PENDING-3 — ГОТОВО (2026-06-13).** «Языки экскурсии» убраны из основного потока внутрь
  «Добавить детали» (тот же job).

- **PENDING-4 — ГОТОВО (2026-06-13).** Темы: 3 видимых + «Ещё темы →» (тот же job).
  Live-проверка: dev.provodnik.app/form отдаёт «Ещё темы» (accept-check GREEN).

- **Бонус (2026-06-13): канонический порядок бейджей** на карточках путешественника и гида:
  [Тип группы] → [Дата] → [Время] → [Кол-во] → [Цена/Бюджет]; цвет типа группы единый
  sky/purple; «Сборная группа» на оффер-карточке рендерится всегда (job badge-canonical-order).
