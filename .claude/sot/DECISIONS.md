...
### ADR-063: /form is the canonical manual request form — icon-toggle layout (provodnik 2026-06-14)
- **Context:** Homepage `/` is now the AI/conversational request flow (`HeroConversation`). We still want users who prefer to fill the request **manually** to have the classic form. The IA refactor (#124, `ea1132ff`) treated `/form` as a redundant alias and retired it — added a `/form -> /` 301 in `next.config.ts` and left the `homepage-classic` form orphaned (the PR said "no intended visual change", but `/form` rendered a *distinct* manual UI, not an alias of `/`). Separately, section-4 had earlier (commit `acec1d1e`, "form-layout-pending234-repair") swapped the compact group-mode **icon** for a full-width text button.
- **Decision:** `/form` is a first-class **manual** request surface, linked from the homepage ("Предпочитаете заполнить вручную? — Обычная форма"). Its canonical look is the **icon-toggle layout**: a compact `UserPlus` icon group-mode toggle next to "Сколько вас" (+ tooltip "Сборная группа — другие путешественники могут присоединиться"), budget-per-person back **inside** the group row, and the 3-themes + "Ещё темы" collapse retained. Data path stays on the current DB structure (`createRequestAction` from `@/features/requests/create-request-actions`); only JSX/layout was fixed. The `/form -> /` redirect must NOT exist.
- **Canonical state / recovery:** commit **`fa41b912`** (PR #125, branch `restore/form-icon-manual`). If `/form` is lost or regresses again, recover with: `git checkout fa41b912 -- "src/app/(home)/form/page.tsx" src/features/homepage-classic/components/homepage-request-form-classic.tsx`; ensure `next.config.ts` has **no** `{ source: "/form", destination: "/" }` redirect; ensure the homepage "Обычная форма" link (`src/features/homepage/components/hero-conversation.tsx`) points to `/form` not `/`. The icon section-4 originates from commit `493e2d1a`; the `homepage-request-form.test.tsx` "icon-toggle layout" test guards it.
- **Alternatives considered:** (a) Accept the IA retirement and keep only the conversational flow — rejected; explicit product call to keep a manual fallback. (b) Full revert to `493e2d1a` (also drops the 3-themes collapse) — rejected; keep the approved collapse. (c) Revert the whole `acec1d1e` commit — rejected; bundles unrelated layout work.
- **Consequences / guard:** Do NOT re-collapse `/form` into `/` or re-add the `/form -> /` redirect — the manual form is intentional. This ADR reverses the `/form` portion of #124; coordinate with `s6s8` on future IA sweeps so the route is not deleted again. Keep the hook on `@/features/requests/create-request-actions`.
- **Verification:** `tsc` clean · `eslint`/`lint:ratchet` clean · form test 25/25 · `next build` (`/form` in manifest, redirect gone) · live dev `GET /form` 200 rendering icon + budget + themes.
- **Date:** 2026-06-14
